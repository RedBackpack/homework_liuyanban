    ...
    Init:function(){
        ...
        $(".postBtn").click(function(){
            $.ajax({
                ...
                'method':'get',
                'url':'./json_submit_ly.php?content=' + $(".Input_text").val(),
                'dataType':'json',
                success:function(res){
                    if(res.length === 3 && res['code'] == 200){
                        $('article').append('<p>您的留言成功,刷新后显示</p>');
                    }else{
                        alert(res['msg']);
                    }
                },
                error:function(xx,aa,cc){
                    alert(cc);
                    //alert('系统发生错误,请稍后重试!!!');
                }
            });
